````markdown
<div align="center">

# ⚡ `whoami` -> Thejas Nirmal
### 👨‍💻 CSE Undergraduate | 🛡️ Cybersecurity & IoT | 🚀 Future Entrepreneur

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=FF0000&center=true&vCenter=true&width=700&lines=Cybersecurity+Enthusiast;IoT+Developer;Future+Tech+Entrepreneur;Building+Skills+%26+Startups;Cinematic+Editor+%7C+Gamer+%7C+Creator" />

</div>

---

```bash
user@thejas:~$ whoami
Thejas Nirmal

user@thejas:~$ education --current
BE Computer Science and Engineering
BMS Institute of Technology and Management

user@thejas:~$ interests --list
Cybersecurity
IoT
Database Management
Web3
AI/ML

user@thejas:~$ status
Building skills, launching ventures, and creating cinematic edits.
````

---

# 🌟 About Me

* 🛡️ Passionate about **Cybersecurity, IoT & Web3**
* 💻 Currently learning **Python, MySQL, C & Backend Development**
* 🚀 Future-focused on building **startups and scalable ventures**
* 🎬 Cinematic **Video Editor & Photographer**
* 🎮 Competitive Gamer — *Valorant & Forza Horizon 5*
* ⚙️ Loves experimenting with **hardware & tech systems**

---

# 🛠️ Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=python,c,mysql,linux,git,github,vscode,html,css" />

</p>

---

# 📊 GitHub Analytics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=ThejasNirmal&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=ff0000&icon_color=ff0000" />

<img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=ThejasNirmal&theme=tokyonight&hide_border=true&background=0d1117&ring=ff0000&fire=ff0000&currStreakLabel=ff0000" />

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ThejasNirmal&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=ff0000" />

</div>

---

# ⚡ Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=ThejasNirmal&theme=react-dark&hide_border=true&bg_color=0d1117&color=ff0000&line=ff0000&point=ffffff" />

</div>

---

# 🧠 Currently Exploring

```python
class ThejasNirmal:

    def __init__(self):
        self.role = "CSE Undergraduate"
        self.interests = [
            "Cybersecurity",
            "IoT",
            "Web3",
            "AI/ML"
        ]

    def current_goal(self):
        return "Build impactful ventures & become financially independent"

me = ThejasNirmal()

print(me.current_goal())
```

---

# 🎯 Goals

* 🚀 Launch my own startup
* 🛡️ Master Cybersecurity & IoT
* 💸 Build multiple income streams
* 🌍 Create impactful tech products
* 🏎️ Achieve financial freedom before graduation

---

# 📫 Connect With Me

<p align="center">

<a href="https://www.linkedin.com/in/thejas-nirmal-43418032a/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://www.instagram.com/___the_cockpit_lover___/">
  <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
</a>

<a href="https://thejasnirmal.vercel.app/">
  <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

</p>

---

<div align="center">

### ⚡ *"Code. Build. Scale. Repeat."*

<img src="https://komarev.com/ghpvc/?username=ThejasNirmal&label=Profile+Views&color=ff0000&style=flat" />

</div>
```
